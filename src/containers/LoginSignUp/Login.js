...
const Login = (props) => {
    ...
    const authenticate = (path, data) => {
        ...
        axios.authenticate(path, data).then(resp => {
            setLoading(false)
            if(resp.message){
                ...
                createToast("Successful Login! Welcome " + resp.user.u_uuid ,"success")
            }
    ...
    return (
        ...
        <Card style={{ width: '400px'}} className="my-5 shadow p-3 mb-5 bg-white rounded mx-auto">
            <CardImg src= { pf1 } style={{height:"20em"}}/>
            <CardText>
                <Input type = 'email' placeholder={'E-mail'} label={'Email '} valid={isValidEmail} value = {email} onChange={onEmailChange}/>
